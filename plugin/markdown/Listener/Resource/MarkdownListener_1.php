...
class MarkdownListener implements ContainerAwareInterface {
    ...
    public function onOpen(OpenResourceEvent $event) {
       ...
       $user = $this->container->get('security.token_storage')->getToken()->getUser();
        ...
        $id = array_pop($keys);
        if (empty($id)){
            $id = 'markdown_' . rand(0, 1000000000);
        }
        $formFactory = $this->container->get('form.factory');     
        $form = $formFactory->create(new MarkdownOpenType($id));
        $form->handleRequest($request);
        ...
        if ($form->isSubmitted()) {
            ...
            $tempVal = $varArray[content];
            if(empty($tempVal)){
                $tempVal='please add record';
            }
            
            $revision->setContent($tempVal);
            $revision->setHtmlcontent($varArray[htmlcontent]);
            $revision->setUser($user);
            $revision->setMarkdown($markdown);
            $revision->setVersion($markdown->getVersion() + 1);
            $markdown->setVersion($markdown->getVersion() + 1);
            $em->persist($markdown);
            $em->persist($revision);      
            $em->flush();
       // if ( $form->isValid()) {  }
        } 
      $form = $formFactory->create(new MarkdownOpenType('markdown_' . rand(0, 1000000000)),$markdown);
      ...
      switch($content){
          case 1:
           ...
           case 2:
               $content = $this->container->get('templating')->render(
                'MindmeMarkdownBundle:Markdown:index_normal.html.twig', [
            'form' => $form->createView(),
            'markdown' => $markdown,
            '_resource' => $markdown,
        ]);
           ...
           break;
           case 4:$content = $this->container->get('templating')->render(
